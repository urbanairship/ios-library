        ...
        VStack(spacing: 20) {
            ...
            VStack {
                ...
                HStack {
                    Text("Can Go Back:")
                    Spacer()
                    Text(sceneController.canGoBack ? "✅" : "❌")
                }
                HStack {
                    Text("Can Go Forward:")
                    Spacer()
                    Text(sceneController.canGoForward ? "✅" : "❌")
                }
            }
            .padding()
            ...
            HStack(spacing: 16) {
                Button(action: {
                    _ = sceneController.navigate(request: .back)
                }) {
                    Label("Back", systemImage: "arrow.left")
                        .frame(maxWidth: .infinity)
                }
                .disabled(!sceneController.canGoBack)

                Button(action: {
                    _ = sceneController.navigate(request: .next)
                }) {
                    Label("Next", systemImage: "arrow.right")
                        .frame(maxWidth: .infinity)
                }
                .disabled(!sceneController.canGoForward)
            }
