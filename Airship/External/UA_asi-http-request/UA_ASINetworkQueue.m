...
- (void)dealloc
{
	...
	for (UA_ASIHTTPRequest *request in [self operations]) {
...
- (void)resetProgressDelegate:(id *)progressDelegate
{
	...
	if ([*progressDelegate respondsToSelector:selector]) {
		double max = 1.0;
		[UA_ASIHTTPRequest performSelector:selector onTarget:progressDelegate withObject:nil amount:&max callerToRetain:nil];
	}
	selector = @selector(setDoubleValue:);
	if ([*progressDelegate respondsToSelector:selector]) {
		double value = 0.0;
		[UA_ASIHTTPRequest performSelector:selector onTarget:progressDelegate withObject:nil amount:&value callerToRetain:nil];
	}
#else
...
- (void)addHEADOperation:(NSOperation *)operation
{
	if ([operation isKindOfClass:[UA_ASIHTTPRequest class]]) {
		...
		UA_ASIHTTPRequest *request = (UA_ASIHTTPRequest *)operation;
...
{
	if (![operation isKindOfClass:[UA_ASIHTTPRequest class]]) {
		[NSException raise:@"AttemptToAddInvalidRequest" format:@"Attempted to add an object that was not an UA_ASIHTTPRequest to an UA_ASINetworkQueue"];
	}
	...
	if ([self showAccurateProgress]) {
		...
		if ([[request requestMethod] isEqualToString:@"GET"]) {
			if ([self isSuspended]) {
				UA_ASIHTTPRequest *HEADRequest = [request HEADRequest];
...
- (void)request:(UA_ASIHTTPRequest *)request didReceiveBytes:(long long)bytes
{
	...
	if ([self downloadProgressDelegate]) {
		[UA_ASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self bytesDownloadedSoFar] ofTotal:[self totalBytesToDownload]];
	}
}
...
- (void)request:(UA_ASIHTTPRequest *)request didSendBytes:(long long)bytes
{
	...
	if ([self uploadProgressDelegate]) {
		[UA_ASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self bytesUploadedSoFar] ofTotal:[self totalBytesToUpload]];
	}
}
...
{
	UA_ASINetworkQueue *newQueue = [[[self class] alloc] init];
