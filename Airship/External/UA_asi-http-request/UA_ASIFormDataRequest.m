...
#import "UA_ASIFormDataRequest.h"


// Private stuff
@interface UA_ASIFormDataRequest ()
...
@implementation UA_ASIFormDataRequest
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setPostFormat:UA_ASIURLEncodedPostFormat];
...
- (void)addFile:(NSString *)filePath withFileName:(NSString *)fileName andContentType:(NSString *)contentType forKey:(NSString *)key
{
	...
	if (!fileExists || isDirectory) {
		[self failWithError:[NSError errorWithDomain:UA_NetworkRequestErrorDomain code:UA_ASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"No file exists at %@",filePath],NSLocalizedDescriptionKey,nil]]];
	}
	...
	if (!contentType) {
		contentType = [UA_ASIHTTPRequest mimeTypeForFileAtPath:filePath];
	}
...
- (void)buildPostBody
{
	...
	if ([self postFormat] == UA_ASIURLEncodedPostFormat) {
...
#if DEBUG_FORM_DATA_REQUEST
	UA_ASI_DEBUG_LOG(@"%@",[self debugBodyString]);
...
- (void)buildURLEncodedPostBody
{
	...
	if ([[self fileData] count] > 0) {
		[self setPostFormat:UA_ASIMultipartFormDataPostFormat];
		[self buildMultipartFormDataPostBody];
		return;
	}
...
{
	UA_ASIFormDataRequest *newRequest = [super copyWithZone:zone];
