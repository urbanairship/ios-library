...
- (UA_FMStatement*) cachedStatementForQuery:(NSString*)query {
...
- (BOOL) goodConnection {
    ...
    UA_FMResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void) compainAboutInUse {
    NSLog(@"The UA_FMDatabase %@ is currently in use.", self);
    
    if (crashOnErrors) {
        NSAssert1(false, @"The UA_FMDatabase %@ is currently in use.", self);
    }
}
...
- (id) executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orVAList:(va_list)args {
    ...
    UA_FMResultSet *rs = nil;
    
    int rc                  = 0x00;;
    sqlite3_stmt *pStmt     = 0x00;;
    UA_FMStatement *statement  = 0x00;
...
//#endif
    ...
    if (!statement) {
        statement = [[UA_FMStatement alloc] init];
    ...
    rs = [UA_FMResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (BOOL) executeUpdate:(NSString*)sql withArgumentsInArray:(NSArray*)arrayArgs orVAList:(va_list)args {
    ...
    int rc                   = 0x00;
    sqlite3_stmt *pStmt      = 0x00;
    UA_FMStatement *cachedStmt = 0x00;
...
//#endif
    ...
    if (shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[UA_FMStatement alloc] init];
...
#pragma mark -
#pragma mark FMDatabaseAdditions

#define RETURN_RESULT_FOR_QUERY_WITH_SELECTOR(type, sel)             \
va_list args;                                                        \
va_start(args, query);                                               \
UA_FMResultSet *resultSet = [self executeQuery:query withArgumentsInArray:0x00 orVAList:args];   \
va_end(args);                                                        \
...
- (BOOL)tableExists:(NSString*)tableName {
    ...
    UA_FMResultSet *rs = [self executeQuery:@"select [sql] from sqlite_master where [type] = 'table' and lower(name) = ?", tableName];
...
- (UA_FMResultSet*)getSchema {
    ...
    UA_FMResultSet *rs = [self executeQuery:@"SELECT type, name, tbl_name, rootpage, sql FROM (SELECT * FROM sqlite_master UNION ALL SELECT * FROM sqlite_temp_master) WHERE type != 'meta' AND name NOT LIKE 'sqlite_%' ORDER BY tbl_name, type DESC, name"];
...
- (UA_FMResultSet*)getTableSchema:(NSString*)tableName {
    ...
    UA_FMResultSet *rs = [self executeQuery:[NSString stringWithFormat: @"PRAGMA table_info(%@)", tableName]];
...
- (BOOL)columnExists:(NSString*)tableName columnName:(NSString*)columnName {
    ...
    //get table schema
    UA_FMResultSet *rs = [self getTableSchema: tableName];
