...
class ActionDetailViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func createMessageCell(_ indexPath:IndexPath) -> UITableViewCell {
        ...
        let cell = defaultActionDetailCell(indexPath)

        switch indexPath {
        case nameIdx:
            cell.title.text = "ua_message_action".localized()
            cell.subtitle.text = schedule.dataJSONString
        default:
            break
        }
    ...
    func defaultActionDetailCell(_ indexPath:IndexPath) -> AutomationDetailCell {
