...
public protocol UAJavaScriptCommandDelegate {
    ...
    ///
    /// If the passed command name is not one the delegate responds to return `false`. If the command is handled, return
    /// `true` and the command will not be handled by another delegate.
    ///
...
public class UAJavaScriptCommandDelegateWrapper: NSObject, JavaScriptCommandDelegate {
    ...
    public func performCommand(_ command: AirshipCore.JavaScriptCommand, webView: WKWebView) -> Bool {
        self.delegate.performCommand(command, webView: webView)
    }
}
