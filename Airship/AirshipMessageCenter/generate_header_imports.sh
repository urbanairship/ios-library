...
TMP_LIB_HEADER="${TMP_LIB}/AirshipMessageCenterLib.h"
SOURCE_LIB_HEADER="${ROOT_PATH}/Source/Public/AirshipMessageCenterLib.h"

# Find all public headers, excluding AirshipMessageCenterLib and UI
...
if [[ -d "${ROOT_PATH}"/Source ]]; then
    find -s "${ROOT_PATH}"/Source -type f -name '*.h' ! -name 'AirshipMessageCenterLib.h' ! -name 'AirshipMessageCenter.h' ! -name '*+Internal*.h' ! -path './UI/*' \
    -exec basename {} \; | awk '{print "#import \"" $1"\""}' >> "${TMP_LIB_HEADER}"
fi

# If there's already an AirshipMessageCenterLib.h in the framework headers directory
