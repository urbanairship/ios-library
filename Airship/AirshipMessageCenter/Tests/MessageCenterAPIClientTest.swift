...
final class MessageCenterAPIClientTest: XCTestCase {
    ...
    private let user = MessageCenterUser(username: "username", password: "password")
    ...
    func testCreateUserSuccess() async throws {
        ...
        let response = try await self.client.createUser(withChannelID: "some channel")
        let request = self.session.lastRequest!
        XCTAssertEqual(response.statusCode, 201)
        XCTAssertNotNil(response.result)
        XCTAssertEqual(response.result?.username, "some user id")
        XCTAssertEqual(response.result?.password, "some password")
        XCTAssertEqual("https://device-api.urbanairship.com/api/user/", request.url!.absoluteString)
    ...
    func testUpdateUserSuccess() async throws {
        ...
        XCTAssertNil(response.result)
        XCTAssertEqual("https://device-api.urbanairship.com/api/user/username", request.url!.absoluteString)
        XCTAssertEqual("POST", request.method)
