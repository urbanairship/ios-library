...
public struct MessageCenterListView: View {
    ...
    public init(viewModel: MessageCenterMessageListViewModel) {
        self.source = .external(viewModel)
    }
    ...
    private struct Owned: View {
        ...
        init(predicate: (any MessageCenterPredicate)?) {
            _viewModel = StateObject(wrappedValue: MessageCenterMessageListViewModel(predicate: predicate))
        }
    ...
    /// Renders the list. Always observes whatever view model it's handed, whether that's owned by
    /// ``Owned`` or supplied externally by ``MessageCenterListView``'s caller.
    private struct Content: View {
        ...
        init(viewModel: MessageCenterMessageListViewModel) {
        ...
        private func makeCell(
            ...
            let accessibilityLabel = item.message.accessibilityDescription
        ...
        var body: some View {
            ...
            .airshipOnChangeOf(self.viewModel.messages, initial: true) { messages in
                if messages.isEmpty {
