...
struct MessageCenterThemeLoader {
    ...
    static func defaultPlist() -> MessageCenterTheme? {
        if let _ = try? plistPath(file: messageCenterFileName, bundle: Bundle.main) {
            do {
                return try fromPlist(messageCenterFileName)
            } catch {
                AirshipLogger.error("Unable to load message center theme \(error)")
            }
        }
    ...
    static func fromPlist(_ file: String, bundle: Bundle = Bundle.main) throws -> MessageCenterTheme {
...
fileprivate extension MessageCenterThemeLoader.FontConfig {
...
fileprivate extension MessageCenterThemeLoader.Config {
    func toMessageCenterTheme() throws -> MessageCenterTheme {
        ...
        theme.selectAllButtonTitleColor = self.selectAllButtonTitleColor?.toColor()
        theme.deleteButtonTitleColor = self.deleteButtonTitleColor?.toColor()
        theme.markAsReadButtonTitleColor = self.markAsReadButtonTitleColor?.toColor()
