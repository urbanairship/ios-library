...
if [[ -d "${ROOT_PATH}"/Source ]]; then
    find -s "${ROOT_PATH}"/Source -type f -name '*.h' ! -name 'AirshipAutomationLib.h' ! -name 'AirshipAutomation.h' ! -name '*+Internal*.h' ! -path './UI/*' \
    -exec basename {} \; | awk '{print "#import \"" $1"\""}' >> "${TMP_LIB_HEADER}"
fi

# If there's already an AirshipAutomationLib.h in the framework headers directory
