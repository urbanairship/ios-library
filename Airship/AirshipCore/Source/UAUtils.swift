...
@objc
public class UAConnectionType : NSObject {
...
public class UAUtils : NSObject {
    ...
    public class func connectionType() -> String {
        ...
        guard let reachability = withUnsafePointer(to: &zeroAddress, {
            ...
            return UAConnectionType.none
        }
        
        var flags = SCNetworkReachabilityFlags()
        let isSuccess = SCNetworkReachabilityGetFlags(reachability, &flags)
        
        var connectionType: String = UAConnectionType.none
        ...
        if !flags.contains(.connectionRequired) {
            connectionType = UAConnectionType.wifi
        }
        ...
        if (flags.contains(.isWWAN)) {
            connectionType = UAConnectionType.cell
        }
    ...
    public class func authHeader(username: String, password: String) -> String? {
        ...
        guard let encodedData = UABase64.stringFromData(data) else {
            return nil
        }
    ...
    public class func logFailedRequest(_ request: UARequest?, message: String?, error: NSError?, response: HTTPURLResponse?) {
        let logMessage = """
