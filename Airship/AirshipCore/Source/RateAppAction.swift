...
public class RateAppAction: NSObject, Action {
    ...
    init(appRater: AppRaterProtocol,
...
fileprivate struct DefaultAppRater: AppRaterProtocol {
    @MainActor
    func openStore(itunesID: String) async throws {
        let urlString = "itms-apps://itunes.apple.com/app/id\(itunesID)?action=write-review"
    ...
    func showPrompt() async throws {
        guard let scene = self.findScene() else {
            throw AirshipErrors.error("Unable to find scene for rate app prompt")
        }
