...
public class UAVersionMatcher : NSObject {
    ...
    public init?(versionConstraint: String) {
        let strippedVersionConstraint = UAVersionMatcher.removeWhitespace(versionConstraint)
        self.versionConstraint = versionConstraint

        if let parsedConstraint = UAVersionMatcher.parseExactVersionConstraint(strippedVersionConstraint) {
            self.constraintType = .exactVersion
            self.parsedConstraint = parsedConstraint
        } else if let parsedConstraint = UAVersionMatcher.parseSubVersionConstraint(strippedVersionConstraint) {
            self.constraintType = .subVersion
            self.parsedConstraint = parsedConstraint
        } else if let parsedConstraint = UAVersionMatcher.parseVersionRangeConstraint(strippedVersionConstraint) {
    ...
    public class func matcher(versionConstraint: String) -> UAVersionMatcher? {
        return UAVersionMatcher(versionConstraint: versionConstraint)
    }
    ...
    func versionMatchesRange(_ checkVersion: String) -> Bool {
        ...
        if startBoundary != .infinite {
            ...
            let result = UAUtils.compareVersion(startOfRange, toVersion: checkVersion)
        ...
        if endBoundary != .infinite {
            ...
            let result = UAUtils.compareVersion(checkVersion, toVersion: endOfRange)
            switch endBoundary {
                case .inclusive:
                    if result != .orderedAscending && result != .orderedSame {
                        return false
    ...
    public override func isEqual(_ other: Any?) -> Bool {
        guard let other = other as? UAVersionMatcher else {
            return false
        }
    ...
    func isEqual(to matcher: UAVersionMatcher?) -> Bool {
