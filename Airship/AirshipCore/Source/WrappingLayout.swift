...
internal struct WrappingLayout: Layout {
    ...
    func sizeThatFits(
        ...
        let maxWidth = viewConstraints.width ?? proposal.width ?? WKInterfaceDevice.current().screenBounds.size.width
#else
        // Determine the maximum width from viewConstraints or proposal
        let maxWidth = viewConstraints.width ?? proposal.width ?? UIScreen.main.bounds.width
#endif
