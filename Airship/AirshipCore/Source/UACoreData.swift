...
public actor UACoreData {
    private static let UAManagedContextStoreDirectory = "com.urbanairship.no-backup"
        ...
        let task = Task {
            ...
            if !coreDataPrepared {
                ...
                do {
                    ...
                    AirshipLogger.error("Failed to create store \(error), deleting and trying again.")
                    try deleteStoresOnDisk()
                    try await loadStores(container: container)
                }
    ...
    private func prepareStore() async throws {
        if !inMemory {
            ...
            guard await UIApplication.shared.isProtectedDataAvailable else {
                throw AirshipErrors.error("Protected data unavailable")
            }
    ...
    private func storeSQLDirectory() -> URL? {
        ...
        return baseDirectory?
            .appendingPathComponent(Self.UAManagedContextStoreDirectory)
    }
