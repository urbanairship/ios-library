...
public class UAJSONValueMatcher : NSObject {
    ...
    private var versionMatcher: UAVersionMatcher?
    private var arrayPredicate: UAJSONPredicate?
    ...
    public func payload() -> [String : Any] {
        ...
        payload[UAJSONValueMatcher.equalsKey] = equals
        payload[UAJSONValueMatcher.atLeastKey] = atLeast
        payload[UAJSONValueMatcher.atMostKey] = atMost
        payload[UAJSONValueMatcher.isPresentKey] = isPresent
        payload[UAJSONValueMatcher.versionConstraintKey] = versionConstraint
        payload[UAJSONValueMatcher.arrayIndexKey] = arrayIndex
        payload[UAJSONValueMatcher.arrayContainsKey] = arrayPredicate?.payload()
    ...
    public class func matcherWhereNumberAtLeast(atLeast number: NSNumber) -> UAJSONValueMatcher {
        let matcher = UAJSONValueMatcher()
    ...
    public class func matcherWhereNumberAtLeast(atLeast lowerNumber: NSNumber, atMost higherNumber: NSNumber) -> UAJSONValueMatcher {
        let matcher = UAJSONValueMatcher()
    ...
    public class func matcherWhereNumberAtMost(atMost number: NSNumber) -> UAJSONValueMatcher {
        let matcher = UAJSONValueMatcher()
    ...
    public class func matcherWhereNumberEquals(_ number: NSNumber) -> UAJSONValueMatcher {
        let matcher = UAJSONValueMatcher()
    ...
    public class func matcherWhereBooleanEquals(_ boolean: Bool) -> UAJSONValueMatcher {
        let matcher = UAJSONValueMatcher()
    ...
    public class func matcherWhereStringEquals(_ string: String) -> UAJSONValueMatcher {
        let matcher = UAJSONValueMatcher()
    ...
    public class func matcherWhereValueIsPresent(_ present: Bool) -> UAJSONValueMatcher {
        let matcher = UAJSONValueMatcher()
    ...
    public class func matcherWithVersionConstraint(_ versionConstraint: String) -> UAJSONValueMatcher? {
        guard let versionMatcher = UAVersionMatcher.matcher(versionConstraint: versionConstraint) else {
            return nil
        }

        let valueMatcher = UAJSONValueMatcher()
    ...
    public class func matcherWithArrayContainsPredicate(_ predicate: UAJSONPredicate) -> UAJSONValueMatcher? {
        let matcher = UAJSONValueMatcher()
    ...
    public class func matcherWithArrayContainsPredicate(_ predicate: UAJSONPredicate, at index: Int) -> UAJSONValueMatcher? {
        let matcher = UAJSONValueMatcher()
        matcher.arrayPredicate = predicate
    ...
    public class func matcherWithJSON(_ json: Any?) throws -> UAJSONValueMatcher {
        ...
        if self.isEqualMatcherExpression(parsedJson) {
            let matcher = UAJSONValueMatcher()
            matcher.equals = parsedJson[UAJSONValueMatcher.equalsKey]
            return matcher
        }

        if self.isNumericMatcherExpression(parsedJson) {
            let matcher = UAJSONValueMatcher()
            matcher.atMost = parsedJson[UAJSONValueMatcher.atMostKey] as? NSNumber
            matcher.atLeast = parsedJson[UAJSONValueMatcher.atLeastKey] as? NSNumber
            return matcher
        }

        if self.isPresentExpression(parsedJson) {
            return self.matcherWhereValueIsPresent((parsedJson[UAJSONValueMatcher.isPresentKey] as? NSNumber)?.boolValue ?? false)
        }

        if self.isArrayMatcherExpression(parsedJson) {
            let matcher = UAJSONValueMatcher()
            matcher.arrayPredicate = try UAJSONPredicate.fromJson(json: parsedJson[UAJSONValueMatcher.arrayContainsKey])
            matcher.arrayIndex = parsedJson[UAJSONValueMatcher.arrayIndexKey] as? NSNumber
            return matcher
        }

        if let constraint = parsedJson[UAJSONValueMatcher.versionConstraintKey] as? NSString {
            if let matcher = self.matcherWithVersionConstraint(constraint as String) {
                return matcher
            }
        }

        if let constraint = parsedJson[UAJSONValueMatcher.alternateVersionConstraintKey] as? NSString {
            if let matcher = self.matcherWithVersionConstraint(constraint as String) {
                return matcher
            }
        }
    ...
    class func isEqualMatcherExpression(_ expression: [AnyHashable : Any]) -> Bool {
        ...
        return expression[UAJSONValueMatcher.equalsKey] != nil
    }
    ...
    class func isNumericMatcherExpression(_ expression: [AnyHashable : Any]) -> Bool {
        ...
        if (expression.count) == 1 {
            return (expression[UAJSONValueMatcher.atLeastKey] is NSNumber) || (expression[UAJSONValueMatcher.atMostKey] is NSNumber)
        }
    ...
    class func isPresentExpression(_ expression: [AnyHashable : Any]) -> Bool {
        ...
        let subexp = expression[UAJSONValueMatcher.isPresentKey]
    ...
    class func isArrayMatcherExpression(_ expression: [AnyHashable : Any]) -> Bool {
        ...
        if (expression.count) == 1 {
            return expression[UAJSONValueMatcher.arrayContainsKey] is [AnyHashable : Any]
        }

        return (expression[UAJSONValueMatcher.arrayContainsKey] is [AnyHashable : Any]) && (expression[UAJSONValueMatcher.arrayIndexKey] is NSNumber)
    }

    /// NOTE: For internal use only. :nodoc:
    public override func isEqual(_ other: Any?) -> Bool {
        guard let matcher = other as? UAJSONValueMatcher? else {
            return false
        }
    ...
    func isEqual(to matcher: UAJSONValueMatcher?) -> Bool {
        if let equals = equals {
