...
class ContactAPIClient: ContactsAPIClientProtocol {
    ...
    func resolve(
        ...
        let request = self.request(
            payload,
            "\(config.deviceAPIURL ?? "")\(ContactAPIClient.path)/resolve"
        )
        ...
        return session.performHTTPRequest(
            ...
            completionHandler: { (data, response, error) in
                guard let response = response else {
                    AirshipLogger.debug(
                        "Resolving contact finished with error: \(error.debugDescription)"
                    )
                ...
                if response.statusCode == 200 {
                    do {
                        guard data != nil else {
                            completionHandler(
                                nil,
                                AirshipErrors.parseError("Missing body")
                            )
                            return
                        }
                        ...
                        guard
                            ...
                            completionHandler(
                                nil,
                                AirshipErrors.parseError("Missing contact_id")
                            )
                            return
                        }
                        guard
                            ...
                            completionHandler(
                                nil,
                                AirshipErrors.parseError("Missing is_anonymous")
                            )
                            return
                        }
    ...
    func identify(
        ...
        let request = self.request(
            payload,
            "\(config.deviceAPIURL ?? "")\(ContactAPIClient.path)/identify"
        )
        ...
        return session.performHTTPRequest(
            ...
            completionHandler: { (data, response, error) in
                ...
                guard let response = response else {
                    AirshipLogger.debug(
                        "Identifying contact finished with error: \(error.debugDescription)"
                    )
                    completionHandler(nil, error)
                    return
                }
    ...
    func reset(
        ...
        let request = self.request(
            payload,
            "\(config.deviceAPIURL ?? "")\(ContactAPIClient.path)/reset"
        )
        ...
        return session.performHTTPRequest(
            ...
            completionHandler: { (data, response, error) in
                guard let response = response else {
                    AirshipLogger.debug(
                        "Resetting contact finished with error: \(error.debugDescription)"
                    )
                    completionHandler(nil, error)
                    return
                }
    ...
    func update(
        ...
        let request = self.request(
            payload,
            "\(config.deviceAPIURL ?? "")/api/contacts/\(identifier)"
        )
    ...
    func associateChannel(
        ...
        return session.performHTTPRequest(request) { data, response, error in
            guard let response = response else {
                AirshipLogger.debug(
                    "Associate channel finished with error: \(error.debugDescription)"
                )
                completionHandler(nil, error)
                return
            }
            ...
            if response.statusCode == 200 {
                ...
                let regResponse = ContactAssociatedChannelResponse(
                    status: response.statusCode,
                    channel: channel
                )
    ...
    func registerEmail(
        ...
        let request = self.request(
            payload,
            "\(config.deviceAPIURL ?? "")\(ContactAPIClient.channelsPath)/restricted/email"
        )
    ...
    func registerSMS(
        ...
        let request = self.request(
            payload,
            "\(config.deviceAPIURL ?? "")\(ContactAPIClient.channelsPath)/restricted/sms"
        )
    ...
    func registerOpen(
        ...
        let request = self.request(
            payload,
            "\(config.deviceAPIURL ?? "")\(ContactAPIClient.channelsPath)/restricted/open"
        )
    ...
    private func registerChannel(
        ...
        requestDisposable = session.performHTTPRequest(
            request,
            completionHandler: { (data, response, error) in
                guard let response = response else {
                    AirshipLogger.debug(
                        "Creating \(channelType) channel finished with error: \(error.debugDescription)"
                    )
                    completionHandler(nil, error)
                    return
                }
                ...
                do {
                    guard let channelID = try self.parseChannelID(data: data)
                    else {
                        completionHandler(
                            nil,
                            AirshipErrors.error("Missing channel ID")
                        )
                        return
                    }
                    ...
                    lock.sync {
                        if isDisposed {
                            completionHandler(
                                nil,
                                AirshipErrors.error("cancelled")
                            )
                        } else {
                            requestDisposable = self.associateChannel(
    ...
    func fetchSubscriptionLists(
        ...
        let request = Request(builderBlock: { [self] builder in
            ...
            builder.url = URL(
                string:
                    "\(config.deviceAPIURL ?? "")\(ContactAPIClient.subscriptionListPath)\(identifier)"
            )
            builder.username = config.appKey
            builder.password = config.appSecret
            builder.setValue(
        ...
        return session.performHTTPRequest(
            ...
            completionHandler: { (data, response, error) in
                guard let response = response else {
                    AirshipLogger.debug(
                        "Retrieving subscription lists associated with a contact finished with error: \(error.debugDescription)"
                    )
                    completionHandler(nil, error)
                    return
                }
                ...
                if response.statusCode == 200 {
                    ...
                    do {
                        ...
                        let clientResponse =
                            ContactSubscriptionListFetchResponse(
                                response.statusCode,
                                scopedLists
                            )
                    ...
                    let clientResponse = ContactSubscriptionListFetchResponse(
                        response.statusCode
                    )
                    completionHandler(clientResponse, nil)
                }
