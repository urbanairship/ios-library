...
public class ChannelAPIClient: NSObject {
    ...
    public func createChannel(
        ...
        guard let data = try? payload.encode() else {
            completionHandler(
                nil,
                AirshipErrors.error("Unable to encode data.")
            )
            return Disposable()
        }

        AirshipLogger.debug(
            "Creating channel with: \(String(data:data, encoding: .utf8) ?? "")"
        )
        ...
        return session.performHTTPRequest(
            ...
            completionHandler: { (data, response, error) in
                guard let response = response else {
                    AirshipLogger.debug(
                        "Channel creation finished with error: \(error.debugDescription)"
                    )
                ...
                if status == 200 || status == 201 {
                    do {
                        ...
                        guard
                            let channelID = jsonResponse?["channel_id"]
                                as? String
                        else {
                            completionHandler(
                                nil,
                                AirshipErrors.parseError("Missing channel_id")
                            )
                            return
                        }

                        let response = ChannelCreateResponse(
                            status: status,
                            channelID: channelID
                        )
    ...
    public func updateChannel(
        ...
        AirshipLogger.debug(
            "Updating channel with: \(String(data:data, encoding: .utf8) ?? "")"
        )

        let channelLocation =
            "\(config.deviceAPIURL ?? "")\(self.path)\(channelID)"
        ...
        return session.performHTTPRequest(
            ...
            completionHandler: { (data, response, error) in
                guard response != nil else {
                    AirshipLogger.debug(
                        "Channel update finished with error: \(error.debugDescription)"
                    )
                    completionHandler(nil, error)
                    return
                }
                ...
                AirshipLogger.debug(
                    "Channel update finished with response: \(response!)"
                )
                completionHandler(
