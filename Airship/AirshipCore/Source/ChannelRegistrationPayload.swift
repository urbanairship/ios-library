...
public class ChannelRegistrationPayload: NSObject, Codable, NSCopying {
    ...
    @objc
    public func minimizePayload(previous: ChannelRegistrationPayload?)
        -> ChannelRegistrationPayload
    {
        let minPayload: ChannelRegistrationPayload =
            self.copy() as! ChannelRegistrationPayload
        ...
        if channel.contactID == previousChannel.contactID {
            if channel.language == previousChannel.language {
                channel.language = nil
            }
            if channel.country == previousChannel.country {
                channel.country = nil
            }
            if channel.timeZone == previousChannel.timeZone {
                channel.timeZone = nil
            }
            if channel.appVersion == previousChannel.appVersion {
                channel.appVersion = nil
            }
            if channel.sdkVersion == previousChannel.sdkVersion {
                channel.sdkVersion = nil
            }
            if channel.isLocationEnabled == previousChannel.isLocationEnabled {
                channel.isLocationEnabled = nil
            }
            if channel.deviceModel == previousChannel.deviceModel {
                channel.deviceModel = nil
            }
            if channel.deviceOS == previousChannel.deviceOS {
                channel.deviceOS = nil
            }
            if channel.carrier == previousChannel.carrier {
                channel.carrier = nil
            }
        }

        if let iOSChannelSetting = channel.iOSChannelSettings,
            let previousiOSChannelSetting = previous.channel.iOSChannelSettings
        {

            if iOSChannelSetting.isScheduledSummary
                == previousiOSChannelSetting.isScheduledSummary
            {
                iOSChannelSetting.isScheduledSummary = nil
            }

            if iOSChannelSetting.isTimeSensitive
                == previousiOSChannelSetting.isTimeSensitive
            {
    ...
    static func == (
        lh: ChannelRegistrationPayload,
        rh: ChannelRegistrationPayload
    ) -> Bool {
    ...
    public class ChannelInfo: NSObject, Codable, NSCopying {
        ...
        init(_ payload: ChannelInfo) {
            ...
            self.iOSChannelSettings =
                payload.iOSChannelSettings?.copy()
                as! ChannelRegistrationPayload.iOSChannelSettings?
        ...
        static func == (lh: ChannelInfo, rh: ChannelInfo) -> Bool {
            return
                lh.deviceType == rh.deviceType && lh.isOptedIn == rh.isOptedIn
                && lh.isBackgroundEnabled == rh.isBackgroundEnabled
                && lh.pushAddress == rh.pushAddress && lh.setTags == rh.setTags
                && lh.tags == rh.tags && lh.tagChanges == rh.tagChanges
                && lh.language == rh.language && lh.country == rh.country
                ...
                && lh.carrier == rh.carrier && lh.contactID == rh.contactID
    ...
    public class IdentityHints: NSObject, Codable, NSCopying {
        ...
        static func == (lh: IdentityHints, rh: IdentityHints) -> Bool {
            return lh.userID == rh.userID
                && lh.accengageDeviceID == rh.accengageDeviceID
        }
    }
