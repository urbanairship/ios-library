...
bash add.sh ios-location ../build/staging/Documentation/AirshipLocationKit $LIB_VERSION
bash add.sh ios-extensions ../build/staging/Documentation/AirshipAppExtensions $LIB_VERSION

git add .
git commit -a -m "Added docs for ios, ios-location & ios-extensions ${LIB_VERSION}"
